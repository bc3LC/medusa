...
vignette: >
  %\VignetteIndexEntry{Energy poverty indices}
...
`medusa` allows to calculate different energy poverty indices. For **Spain**, indices are available for the full time series **2006–2021** using the data included in the package. For **EU countries**, indices can be calculated for the available HBS waves (**2010, 2015 and 2020**), using Eurostat microdata processed with `hbs_eu()`. In both cases, the thresholds used in each index (e.g. national medians) are calculated at the **member state level**.

 The indicators covered by medusa have been commonly used in the literature to measure energy poverty during the last decades and can be useful to assess the dimension of energy poverty, as well as to monitor policies aimed at alleviating this problem.
