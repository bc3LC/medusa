...
in the EU based in one or more socioeconomic or demographic variable
(one plot per variable).

## Usage

``` r
basic_graph_eu(data, var = graph_labels_eu$VARIABLE)
```
...
- var:
  ...
  graph_labels_eu\$VARIABLE (by default) creates a graph with the
  ...
  function is available run \`available_var_impact_eu()\`.
